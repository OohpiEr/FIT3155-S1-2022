...
class Edge:
    def __init__(self, u, v, w, ref) -> None:
        self.u = u
        self.v = v
        self.w = w
        self.ref = ref
...
class Graph:
    ...
    def add_edges(self, argv_edges, argv_direct=False):
        for edge in argv_edges:
            ...
            w = edge[2]
            # add u to v
            current_edge = Edge(self.vertices[u], self.vertices[v], w, edge)
            current_vertex = self.vertices[u]
            current_vertex.add_edge(current_edge)
            # add v to u
            if not argv_direct:
                current_edge = Edge(self.vertices[v], self.vertices[u], w, edge)
        ...
        while len(discovered) > 0:
            ...
            for edge in u.edges:
                ...
                if v.discovered == False:
                    ...
                    path[v.id] = path[u.id] + [(edge.w, edge.ref)]
        return max(path[target.id])
...
class Mst:
    ...
    def get_2nd_mst(self):
        ...
        for e in self.skipped_edges:
            ...
            # print("target {} source {}".format(e[1], e[0]))
            k = g.dfs(g.vertices[e[0]], g.vertices[e[1]])
            print(k)
            ...
            if diff < delta_T:
                ...
                e_new = e
                e_old = k[1]
                mst2_w = self.weight + k[0] - e[2]
            for i in range(len(mst2)):
                # if e[0] == e_old[0] and e[1] == e_old[1]:
                if mst2[i] == e_old:
                    mst2[i] = e_new
                    break
        return (mst2_w, mst2)
...
if __name__ == "__main__":
    graph_file = read_graph_file("graph.txt")
    # graph_file = sys.argv[1]   # text_file_name = "text.txt"
    num_vertices = graph_file[0]
    edges = graph_file[2]
    mst = Mst(num_vertices)
    mst.add_mst(num_vertices, edges)
    mst1_w, mst1 = mst.weight, mst.edges
    mst2_w, mst2 = mst.get_2nd_mst()
    write_output(mst1, mst1_w, mst2, mst2_w)
    # print(g)
