    ...
    while (text_pointer <= n - m):
        ...
        # if mismatch at pat_pointer
            ...
            if good_suffix_arr[pat_pointer + 1][0] > 0:
                ...
                # if mismatch at character before suffix after shift
                if pattern[pat_pointer - shift_good_char] != text[text_pointer + pat_pointer]:
                    # look for match
                    found = False
                    i = 0
                    while not found and i < len(good_suffix_arr[pat_pointer + 1]) - 1:
                    ...
                    # if no match found shift by length of string
                    if not found:
                        shift_good_char = matched_pref_shift(0, matched_prefix_arr, pat_pointer,m)
...
def matched_pref_shift(shift_good_char, matched_prefix_arr, pat_pointer,m):
    if len(matched_prefix_arr) > pat_pointer + 1:
        shift_good_char = m - matched_prefix_arr[pat_pointer + 1]
    else:
        shift_good_char = 1
    return shift_good_char
